...
def display_image():
    ...
    BASE_PATH = "C:/PoseEstimationForMobile/ai_challenger"
...
def run_with_frozen_pb(img_path, input_w_h, frozen_graph, output_node_names):
    ...
    #output = graph.get_tensor_by_name("%s:0"%"Conv2d_0/conv/Conv2D")
    #output = graph.get_tensor_by_name("%s:0"%"Conv2d_0/conv/BatchNorm/FusedBatchNorm")
    #output = graph.get_tensor_by_name("%s:0"%"Conv2d_0/conv/Relu6")
    output = graph.get_tensor_by_name("%s:0"%"Conv2d_1_dw/dw/depthwise_weights/read")
    #output = graph.get_tensor_by_name("%s:0"%"Conv2d_1_dw/dw/depthwise")
	
    image_0 = cv2.imread(img_path)
    w, h, _ = image_0.shape
    if w != input_w_h or h != input_w_h:
        image_ = cv2.resize(image_0, (input_w_h, input_w_h), interpolation=cv2.INTER_LINEAR)
    else:
        image_ = image_0
    #print(image_)
    #image_ = np.ones([input_w_h,input_w_h,3])
    #image_[0][:][:]=2
    with tf.Session() as sess:
        heatmaps = sess.run(output, feed_dict={image: [image_]})
        H,W,C,N=heatmaps.shape
        for h in range(H):
            for w in range(W):
               print('[%d,%d,:,0]'%(h,w))
               print(heatmaps[h,w,:,0])
        """
